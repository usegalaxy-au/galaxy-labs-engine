...
Usage:
    # From a file with one tool ID per line (comments with '#' allowed):
    python fetch_tool_inputs.py tools.txt

    # Or pass tool IDs as command line args:
    python fetch_tool_inputs.py upload1 toolshed.../fastp/fastp

    # Override the servers to query:
    python fetch_tool_inputs.py --server https://usegalaxy.org.au tools.txt
"""
import argparse
import json
import subprocess
...
def main() -> int:
    ...
    for tool_id, data in results:
        print(format_tool_yaml(tool_id, data))
    return 0
